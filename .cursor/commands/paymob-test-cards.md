...
3. `.agents/skills/paymob-integration/references/` — this Escrowd repo
4. `skills/paymob-integration/references/` — repository checkout, relative to the working directory

`${CLAUDE_PLUGIN_ROOT}` is substituted only for plugin installs. If you see it unexpanded, you are not in one — use path 2, 3, or 4. If none resolve, search for `test-credentials.md` under any `paymob-integration` directory.
